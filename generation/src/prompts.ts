...
// 去 AI 腔（對齊全站統一清單；佛法白話尤忌拔高昇華）——各 L2 生成提示尾綴。
const ANTI_AI = `
文風（去 AI 腔，佛法白話尤忌拔高昇華）：
- 不用破折號（——）下定義或插補；要解釋就直接寫。
- 不用「不是X，而是Y」「不僅…更」「並非…而是」的對比套式；要對比就正面直述。
- 不用「值得注意的是」「換句話說」「總而言之」等空轉套語與空泛收束。
- 不用模板化開場（如「在…的今天」「隨著…」）；直接進入內容。
- 據實平述，不替經文加感嘆、拔高或昇華。`;
...
不得加任何說明文字、不得用範圍符號（–／~）、不得自創 id。` + ANTI_AI;
...
規則：只據提供資料；不裁決教義。
sources 只能是提供清單中的「確切 segment_id」（例：mn10:2.1），不得加說明、不得用範圍、不得自創 id。` + ANTI_AI;
...
- 不裁決教義詮釋；該字若涉宗派理解差異，陳述「傳統上有不同理解」並止。` + ANTI_AI;
...
- 不裁決教義；涉詮釋處陳述「傳統上有不同理解」並止。` + ANTI_AI;
