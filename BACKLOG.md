...
## J. 成長內容待辦（流量風向觸發，2026-07-14）

> 用戶定調：**先讓數據跑、有流量風向時才做**，不預先投入。風向來源＝每週一週報的 🔬 情境垂直對比區、reflect 每日觀察項（`vertical-compare-4`、`lexicon-reindex-occ8-outcome`）。策略脈絡見記憶 sutta-io-growth-strategy／sutta-io-audience-chinese-only。

| ID | 項目 | 狀態 | 觸發條件與說明 |
|----|------|------|------|
| J-1 | en 主題頁 answer-first 化（marana/facing-fear/facing-criticism/letting-go 等英文版仍是舊冷內容、已與中文版分歧） | ⬜ 暫緩 | **觸發**：GSC 英文查詢或 `/en/` 頁出現實質曝光/點擊。**現況**：依「受眾是中文使用者」原則暫不投入（英文端需求已被 SuttaCentral/ATI 巨頭滿足）。除非數據反證，維持暫緩。 |
| J-2 | 擴充情境垂直（負面念頭 MN20、慾望 MN13，及更多苦的家族） | ⬜ 待風向 | **觸發**：4 垂直對比（marana/facing-fear/facing-criticism/letting-go）出現贏家——某家族「排得上＋有人讀」。屆時**複製勝出家族的模式**到新垂直，而非平均擴張。落後家族先檢討查詢意圖或換錨經。 |
| J-3 | 補 7 部經 L2 概要/研經卡（mn26/31/34/37/40/41/42） | ⬜ 待需要 | **觸發**：其中某經成為某個「有流量主題」的錨經、或被主題頁引用而需要時，**優先補該經**（比照 MN87 流程：sonnet sub-agent 產、勿用 claude -p）。非全面補完，按需觸發。 |
